...
from solutions.hw89 import metric


class DCGANTrainer:

    def __init__(self, discriminator, generator, optimizer_d, optimizer_g, latent_size=100,
                 device='cuda', metrics_dir='metrics', save_root='ckpt', log_dir=None, load_weights=False,
                 load_epoch=29):
    ...
    def load(self, epoch):
        self.net_g.load_state_dict(torch.load(os.path.join(self.save_root, f'generator_epoch_{epoch}.pt')))
        self.net_d.load_state_dict(torch.load(os.path.join(self.save_root, f'discriminator_epoch_{epoch}.pt')))
